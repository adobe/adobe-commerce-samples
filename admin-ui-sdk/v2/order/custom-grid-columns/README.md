# Order Custom Grid Columns — Admin UI SDK V2

Adds custom columns to the order grid in the Adobe Commerce Admin panel.

When a merchant opens **Sales → Orders**, Commerce calls your App Builder runtime action with the IDs of the visible orders. Your action returns the column values; Commerce renders them inline alongside the built-in columns.
...
1. Merchant opens the order grid in the Commerce Admin
2. Commerce sends a POST request to your `get-order-grid-columns` runtime action with `{ requestId, gridType: "order", ids: ["000000001", "000000002", ...] }`
3. Your action looks up the column values for each order ID and returns them
...
- [Admin UI SDK — Custom Grid Columns](https://developer.adobe.com/commerce/extensibility/admin-ui-sdk/extension-points/order/grid-columns/)
