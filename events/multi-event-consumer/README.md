...
This sample declares three Commerce events: order placed, customer saved, product saved, all routing to a single `consumer` action. The consumer matches the incoming event against the ones it knows about and forwards it, via an OpenWhisk invoke, to the action that actually handles it (fire and forget).
...
| `src/commerce-extensibility-1/lib/router.js`                          | The reusable, domain-agnostic dispatcher: `withAsyncEventRouter(routes)` matches an I/O event code against the incoming CloudEvent `type` and invokes the target action via `openwhisk` (fire and forget). |
| `src/commerce-extensibility-1/actions/consumer/index.js`              | The shared action all three events route to. Resolves each declared event name to its I/O event code (`resolveIoEventCode`) and wraps `withAsyncEventRouter` with the resulting code → action routes.      |
...
3. In `actions/consumer/index.js`, resolve its event code with `resolveEventCode` and add a `{ code, action }` entry to the `withAsyncEventRouter` call.
