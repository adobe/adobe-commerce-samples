...
/**
 * Gets a configuration value from aio-lib-state storage
...
/**
 * Sets a configuration value in aio-lib-state storage
...
export async function setConfigValue(key, value) {
  const state = await init();
  await state.put(key, value, { ttl: -1 }); // -1 means no expiration
}

/**
 * Deletes a configuration value from aio-lib-state storage
