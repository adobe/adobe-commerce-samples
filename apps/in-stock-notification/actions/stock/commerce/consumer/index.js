...
async function main(params) {
  const logger = Core.Logger("stock-commerce-consumer", {
    ...
    switch (params.type) {
      case `com.adobe.commerce.${params.EVENT_PREFIX}.observer.cataloginventory_stock_item_save_commit_after`: {
        ...
        const res = await openwhiskClient.invokeAction(
          "stock-commerce/updated",
          params.data.value,
        );
        response = res?.response?.result?.body;
        statusCode = res?.response?.result?.statusCode;
        const isInStock = params.data?.value?.is_in_stock;
        const inStock =
          isInStock === true ||
          isInStock === "1" ||
          (typeof isInStock === "string" && isInStock.toLowerCase() === "true");
        if (inStock) {
          logger.info("Invoking back-in-stock notifications");
          await openwhiskClient.invokeAction(
            "notify-out-of-stock/back-in-stock",
            params.data.value,
          );
        }
        break;
